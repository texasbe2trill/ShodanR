...
-   [Shodan API Base URL ](https://api.shodan.io/shodan/)
...
According to the Shodan dataset, **{country_text}** are the countries with the highest number of ransomware infections, with **{max(common_country_count)}** incidents.

There are a total of **{nrow(shodan_df_ransomware)}** ransomware infections worldwide!

### Statistical Analysis
- The **mean** number of ransomware infections per country is **{round(mean(common_country_count), 2)}**.
...
kable(common_country_city_count, caption = "Distribution of Ransomware Infections by Country and City",
...
# Create a world map of ransomware infections
 m <- ggplot(shodan_count, aes(x = Longitude, y = Latitude, color = `Country`, size = n)) +
  ...
  geom_point() +
  theme_map() +
  labs(title = "The Global Impact of Ransomware Infections",
